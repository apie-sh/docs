...
Start at the lowest tier that fits your runtime. Add metadata as you move toward production guard mode.
...
Action and resource types are **inferred from tool names** when omitted — fine for monitor mode. Add explicit metadata before enabling guard mode in production. See [Instrument tool calls](/observe/instrument-tool-calls).

**Best for:** Custom agents, production guard enforcement, and compliance-sensitive workloads.
...
4. Switch to **guard mode** and add explicit `action`, `resource`, and `riskLevel` metadata
