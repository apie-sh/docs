...
A pipeline session replay with orchestrator run, LLM tool call (medium risk), and MCP pipeline trigger (high risk). In monitor mode, guard evaluations show what would be blocked in guard mode.
...
3. Switch to [guard mode](/guardrails/enforce-guardrails)
