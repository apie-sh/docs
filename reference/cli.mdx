...
```bash
npx apie send-test-event [--mode pipeline|single]
apie send-test-event [--mode pipeline|single]
```

| Flag | Default | Description |
| --- | --- | --- |
| `--mode` | `pipeline` | `pipeline` (full session) or `single` (one tool call) |
...
| `--release-mode` | from config | `monitor` or `guard` |
