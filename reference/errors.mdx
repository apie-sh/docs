...
description: Apie error types, guard failure modes, and MCP JSON-RPC error codes.
---

## ApieError

The SDK throws `ApieError` for client-side failures: registration errors, guard blocks, approval timeouts, and ingest failures.
...
When the guard API fails (network error, timeout), `guardFailureMode` controls behavior:
...
In guard mode, `withTool` throws when:
...
In monitor mode, no errors are thrown for guard decisions.

## MCP JSON-RPC error codes

When using the MCP proxy in guard mode:
